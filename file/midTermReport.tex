...
\begin{tikzpicture}[
  ...
  % Shared
  \node[comp, below=2.5cm of adminpkg, fill=successgreen!10, draw=successgreen] (sidebar) {Sidebar (components/Sidebar.jsx)};
  \node[comp, right=0.5cm of sidebar, fill=successgreen!10, draw=successgreen] (topbar) {Topbar (components/Topbar.jsx)};
  ...
  % Layout
  \node[comp, below=3.5cm of adminpkg, fill=warningamber!10, draw=warningamber] (layout) {AdminLayout (layouts/AdminLayout.jsx)};
...
\centering
\begin{tikzpicture}[
  actor/.style={rectangle, draw=primaryblue, fill=primaryblue!8, rounded corners=6pt, minimum width=2.5cm, minimum height=0.8cm, align=center, font=\scriptsize\bfseries},
  arrow/.style={-{Stealth[length=2.5mm]}, draw=primaryblue!60}
]
  \node[actor] (admin) at (0, 0) {1: Admin};
  \node[actor] (ui) at (4, 0) {2: Upload UI};
  \node[actor] (backend) at (8, 0) {3: API Server};
  \node[actor] (db) at (8, -3) {4: Database};

  \draw[arrow] (admin) -- node[above, font=\tiny] {1.1 Select File} (ui);
  \draw[arrow] (ui) -- node[above, font=\tiny] {1.2 Send CSV} (backend);
  \draw[arrow] (backend) -- node[right, font=\tiny] {1.3 Insert Records} (db);
  \draw[arrow] (db.west) -| node[above left, font=\tiny, near start] {1.4 Acknowledge} (backend.south west);
  \draw[arrow] (backend.170) -- node[below, font=\tiny] {1.5 Success Res} (ui.10);
  \draw[arrow] (ui.190) -- node[below, font=\tiny] {1.6 Display Status} (admin.350);
\end{tikzpicture}
...
\begin{longtable}{|p{1.5cm}|p{4.5cm}|p{4.5cm}|p{3.5cm}|}
\hline
...
\begin{itemize}
  \item \textbf{Authentication Module}: Admin login interface with basic route protection is functional.
  \item \textbf{Database Schema}: Mongoose models for Students, Semesters, Subjects, and Performance records have been designed and deployed to MongoDB Atlas.
  \item \textbf{Frontend Layouts}: The overarching React application architecture, including the Admin Dashboard wireframes and Guardian Portal layout structure, is integrated using Tailwind CSS.
  \item \textbf{Data Ingestion Foundation}: The backend API to parse and handle form data is set up, though bulk CSV/Excel parsing logic is still being refined.
\end{itemize}
...
\begin{enumerate}
  \item \textbf{Robust Bulk Data Processing}: Finalizing the logic to accurately parse complex, multi-semester Excel/CSV sheets, map subject codes to student profiles, and calculate SGPA/CGPA dynamically without data loss.
  \item \textbf{Automated Email Notification Pipeline}: Integrating Nodemailer with an SMTP service (such as Gmail or SendGrid) to dispatch batch emails securely to guardians without triggering spam filters.
  \item \textbf{Secure Token Generation System}: Developing the cryptographic token generation algorithm to create unique, time-sensitive access links for guardians, ensuring that these tokens expire after a set duration and can be manually revoked by the admin.
  \item \textbf{Data Visualization Integration}: Connecting backend student performance data to Recharts to dynamically render interactive line graphs representing SGPA trends on both the admin and guardian portals.
  \item \textbf{Comprehensive Role-Based Access Control}: Enforcing strict server-side validation to ensure that guardians utilizing access links cannot manipulate URL parameters to view other students' sensitive academic data.
  \item \textbf{Production Deployment}: Configuring deployment pipelines for hosting the frontend Single Page Application and the backend serverless functions on Vercel, encompassing environment variable management and CORS policy enforcement.
\end{enumerate}
