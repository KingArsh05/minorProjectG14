...
export const uploadStudentsData = asyncHandler(async (req, res) => {
  const normalizedUserObject = req.body;

  if (
    !Array.isArray(normalizedUserObject) ||
    normalizedUserObject.length === 0
  ) {
    return res
      .status(400)
      .json(new ApiResponse(null, "No students data provided"));
  }

  const operations = normalizedUserObject.map((u) => ({
    updateOne: {
      filter: {
        crn: u.crn,
        urn: u.urn,
  ...
  return res
    .status(200)
    .json(new ApiResponse(result, "Students data uploaded successfully"));
});
