  ...
  return (
    <div className="mb-6">
      ...
      {label && (
        <label
          htmlFor={id}
          className="block text-[0.78rem] font-semibold text-[#5c6385] tracking-wide uppercase mb-[0.4rem]"
        >
          {label}
        </label>
      )}
      ...
      <div className="relative">
        {icon && (
          <span className="absolute inset-y-0 left-0 flex items-center pl-4 text-[#5c6385]">
            {icon}
          </span>
        )}
        ...
        <input
          ...
          className={`w-full bg-[#161925] border text-[#f0f1fa] py-[0.65rem] rounded-xl text-sm outline-none transition-all placeholder:text-[#5c6385]
            ${icon ? "pl-10 pr-4" : "px-4"}
            ${
              errors && errors[id]
                ? "border-red-500 focus:border-red-500 focus:shadow-[0_0_0_3px_rgba(239,68,68,0.12)]"
                : "border-[#252840] focus:border-[#6366f1] focus:shadow-[0_0_0_3px_rgba(99,102,241,0.12)] hover:border-[#3a3f5c]"
            }`}
          {...register(id, rules)}
        />
        ...
        {type === "password" && (
          <button
            ...
            className="absolute inset-y-0 right-0 flex items-center pr-4 text-[#5c6385] hover:text-[#9ba2c0] transition-colors touch-manipulation"
          >
            {showPassword ? (
              <svg
      ...
      <div className="absolute pt-1">
        {errors && errors[id] && (
          <p className="text-[#f87171] text-[10px] flex items-center gap-1">
            <svg
